...
![cwd](/assets/images/Node/process/1.webp)

### 3. process.argv

获取执行进程后面的参数 返回是一个数组 后面我们讲到命令行交互工具的时候会很有用，各种 cli 脚手架也是使用这种方式接受配置参数例如`webpack`

![argv](/assets/images/Node/process/2.webp)
...
![exit](/assets/images/Node/process/3.webp)
...
![env](/assets/images/Node/process/4.webp)
...
![环境变量](/assets/images/Node/process/5.webp)

![环境变量](/assets/images/Node/process/6.png)

其原理就是如果是 windows 就调用 `set` 如果是 posix 就调用 `export` 设置环境变量
