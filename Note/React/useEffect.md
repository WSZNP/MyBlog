...
**确切说清理函数就是副作用函数运行之前，会清除上一次的副作用函数，也就是`useEffect` 的清理函数会在每次依赖项变化前运行，而不仅仅是组件卸载时。**

根据我们下面的例子可以观察到，当组件卸载和依赖项发生变化时，`useEffect`的副作用函数就会执行。
...
![](/assets/images/React/useEffect/1.png)
