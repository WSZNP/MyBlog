...
![](/assets/images/React/useReducer/1.png)
...
1. reducer 是一个用来根据不同的 action 来更新状态的纯函数。
2. 它接收当前状态 (state) 和一个动作对象 (action)，根据 action.type 来决定如何更新 state。
3. 如果 action.type 是 'increment'，则 count 增加 1；如果是 'decrement'，则 count 减少 1。
4. 如果 action.type 不匹配任何已定义的情况，则抛出一个错误。 App 组件:
