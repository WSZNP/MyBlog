...
> 因为 ts 定义的枚举中的每一个组员默认都是从 0 开始的所以也就是
...
> 定义一个枚举 Types 定义一个接口 A 他有一个属性 red 值为 Types.永遠的神
...
![const枚举](/assets/images/TypeScript/枚举类型/1.png)

### 普通声明编译之后

![普通枚举](/assets/images/TypeScript/枚举类型/2.png)

## 6.反向映射

> 它包含了正向映射（ name -> value）和反向映射（ value -> name）

> 要注意的是 反向映射是数字枚举的独有特性，字符串枚举及异构枚举（混合数字和字符串成员）均不支持此操作。
